  ...
  std::shared_ptr<std::vector<Ort::Value>> Model::run(const Ort::Value& inputs, const Ort::RunOptions& runOptions){
  ...
  std::future<std::shared_ptr<std::vector<Ort::Value>>> Model::runAsync(const Ort::Value& inputs, const Ort::RunOptions runOptions){
    if (this->_session == nullptr)
      throw std::runtime_error("Session is not initialized");
